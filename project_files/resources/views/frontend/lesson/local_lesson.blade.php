                            ...
                            <!-- HTML5 VIDEO TAG -->
                                <div id="my_video" type="video/mp4" style="width: 100%;height: 100%"  >
                                    <video  oncontextmenu="return false;"    style="width: 100%;border: 0.5px solid;" playsinline id="screen"
                                            src="{{route('storage',['courses_files',$course->video,$course->id,null])}}" poster="{{route('storage',['courses_files',$course->photo,$course->id,'photos'])}}"></video>
                                </div>
                        @endif
                    </div>
                            ...
                            <ul  style="width: 100%;margin: 0px">
                                ...
                                @foreach($part->lessons as $lesson)
                                    @if($course->isFree())
                                            ...
                                            <li style="width: 100%" >
                                                <a href="{{route('course.video',[$course->id,$lesson->id])}}" class="justify-content-sm-between">
                                                    <strong>{{$count1}}.{{$count2++}}</strong> <i class="icon-play-circled"></i>
{{--                                                    <small class="badge badge-danger">{{CalculateDuration($course->id,$lesson->video)}} min</small> --}}
                                                    {{$lesson->name}}
                                                </a>
                                            </li>
                                        @else
...
{{--                                                                <small class="badge badge-danger">{{CalculateDuration($course->id,$lesson->video)}} min</small> --}}
                                                        ...
                                                        <li style="width: 100%" >
                                                            <a href="{{route('course.video',[$course->id,$lesson->id])}}" class="justify-content-sm-between">
                                                                <strong>{{$count1}}.{{$count2++}}</strong> <i class="icon-play-circled"></i>
                                                                {{--                                                                <small class="badge badge-danger">{{CalculateDuration($course->id,$lesson->video)}} min</small> --}}
                                                                {{$lesson->name}}     <code class="badge badge-sm badge-danger">(paid)</code>
                                                            </a>
                                                        </li>
            ...
            <div class="container margin_60_35" style="transform: none;">
                <div class="row" style="transform: none;">
                    ...
                    <div class="col-lg-8">
                        ...
                        @auth('web')
                            @if(!$user_rate)
                                @if($course->isFree() or $status=='opened')
