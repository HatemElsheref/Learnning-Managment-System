                ...
                <div>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb p-0 breadcrumb-inverse">
                            ...
                            <li class="breadcrumb-item"><a href="{{route('course.show',$lesson->part->course->id)}}">{{ucfirst($lesson->part->course->name)}}</a></li>
                            <li class="breadcrumb-item"><a href="{{route('course.show',$lesson->part->course->id)}}">{{ucfirst($lesson->name)}}</a></li>
            ...
            <div class="row">
                <div class="col-12 ">
                    <div class="card card-default">
                        ...
                        <div class="card-body">
                            @IFACAN('delete_lesson_reviews')
                            <form method="post" action="{{route('lesson.reviews.multi.delete')}}" id="delete-all-selected-check-box">
                                @csrf
                                @method('delete')
                            </form>
                            @ENDACAN
                            <table id="data-table" class="table nowrap" style="width:100%">
                                ...
                                @foreach($reviews as $item)
                                    <tr>
                                        ...
                                        @IFACAN('delete_lesson_reviews')
                                        <form method='post' action='{{route('lesson.reviews.delete',$item->id)}}' id='remove-department-{{$item->id}}'>
                                            @csrf
                                            @method('delete')
                                        </form>
                                        @ENDACAN
