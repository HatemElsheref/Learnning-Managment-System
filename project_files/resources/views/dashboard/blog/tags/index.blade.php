    ...
    <div class="content-wrapper">
        <div class="content">
            ...
            <div class="breadcrumb-wrapper d-flex align-items-center justify-content-between __web-inspector-hide-shortcut__">
                ...
                <div>
                    @IFACAN('create_tag')
                    <a class="btn btn-sm btn-primary" href="{{route('tag.create')}}">Add New Tag</a>
                    @ENDACAN
                </div>
            </div>
                ...
                <div class="col-12 ">
                    <div class="card card-default">
                        <div class="card-header card-header-border-bottom d-flex justify-content-between">
                            <h2>
                                <span class="mdi mdi-tag"></span> Tags Table
                            </h2>
                        ...
                        <div class="card-body">
                            @IFACAN('delete_tag')
                            <form method="post" action="{{route('tag.multi.delete')}}" id="delete-all-selected-check-box">
                                @csrf
                                @method('delete')
                            </form>
                            @ENDACAN
                                ...
                                @foreach($tags as $item)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @ENDACAN
                                            @IFACAN('update_tag')
                                            <a class='btn btn-sm btn-outline-success' href="{{route('tag.edit',$item->id)}}">Edit</a>
                                            @ENDACAN
                                        </td>
                                        @IFACAN('delete_tag')
                                        <form method='post' action='{{route('tag.destroy',$item->id)}}' id='remove-department-{{$item->id}}'>
                                            @csrf
                                            @method('delete')
                                        </form>
                                        @ENDACAN
