...
@section('title')@lang('dashboard.edit_admin')@endsection
                ...
                <div>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb p-0 breadcrumb-inverse">
                            ...
                            <li class="breadcrumb-item" aria-current="page">@lang('dashboard.edit_exist_admin')</li>
                        </ol>
                    </nav>
                </div>
                <div>
...
{{--                            <h2>@lang('dashboard.edit_exist_admin')</h2>--}}
{{--                        </div>--}}
                        <div class="card-body">
                            @include('dashboard.layouts.error')
                            <form action="{{route('admin.update',$staff->id)}}" method="post" enctype="multipart/form-data" autocomplete="off">
                                @csrf
                                @method('put')
                                <div class="form-group">
                                    <label for="name">@lang('dashboard.admin_name')</label>
                                    <input type="text" class="form-control" id="name" name="name" value="{{$staff->name}}" placeholder="@lang('dashboard.enter') @lang('dashboard.admin_name')">
                                </div>
                                <div class="form-group">
                                    <label for="email">@lang('dashboard.admin_email')</label>
                                    <input type="email" class="form-control" id="email" name="email" value="{{$staff->email}}" placeholder="@lang('dashboard.enter') @lang('dashboard.admin_email')">
                                </div>
                                <div class="form-group">
                                    <label for="phone">@lang('dashboard.admin_phone')</label>
                                    <input type="text" class="form-control" id="phone" name="phone" value="{{$staff->phone}}" placeholder="@lang('dashboard.enter') @lang('dashboard.admin_phone')">
                                </div>
                                <div class="form-group ">
                                    <label>@lang('dashboard.gender')</label>
                                    <label class="control control-radio">@lang('dashboard.male')
                                        <input type="radio" name="gender" value="male" @if('male'==$staff->gender) checked="checked" @endif>
                                        <div class="control-indicator"></div>
                                    </label>
                                    <label class="control control-radio">@lang('dashboard.female')
                                        <input type="radio" name="gender" value="female" @if('female'==$staff->gender) checked="checked" @endif>
                                        <div class="control-indicator"></div>
                                    </label>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <button class="btn  btn-success" style="display:block" onclick="document.getElementById('user_avatar').click();return false;">@lang('dashboard.upload')</button>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <table class="table table-hover table-responsive permission-table" id="permissions">
                                        ...
                                        @foreach($Models as $model=>$values)
                                            <tr>
                                                <td style="margin-bottom: 10px" >{{ucfirst($model)}}</td>
                                                ...
                                                @foreach($Maps as $char=>$value)
                                                    ...
                                                    @else
                                                        ...
                                                        <td>
                                                            <div class="custom-control custom-checkbox mb-3">
                                                                <input type="checkbox"  class="custom-control-input" id="{{strtolower($value)}}_{{strtolower($model)}}" name="permissions[]" value="{{strtolower($value).'_'.strtolower($model)}}" @if(in_array(strtolower($value).'_'.strtolower($model),$permissions)) checked @endif>
                                ...
                                <div class="form-footer pt-4 pt-5 mt-4 border-top">
                                    <button type="submit" class="btn btn-success btn-default">@lang('dashboard.edit')</button>
