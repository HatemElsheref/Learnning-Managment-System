                            ...
                            <table id="data-table" class="table nowrap" style="width:100%">
                                ...
                                @foreach($courses as $item)
                                    <tr>
                                        ...
                                        @endif
                                        <td>
                                            <img class='rounded-circle w-45' style='margin-top: 0px;padding-top:0px;height: 45px' src='{{route('UPLOADED.FILES',['courses_files',$item->photo,$item->id,'photos'])}}' alt='course main image'>
                                        </td>
                                        <td>{{$item->id}}</td>
                                        <td>{{$item->name}}</td>
                                        <td>{{$item->code}}</td>
                                        ...
                                        <td>
                                            {{ucfirst($item->hosting)}}
                                        </td>
                                            <td class="text-right">
                                                <div class="dropdown show d-inline-block widget-dropdown">
                                                    <a class="dropdown-toggle icon-burger-mini" href="" role="button" id="dropdown-recent-order1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-display="static"></a>
                                                    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order1">
                                                        <li class="dropdown-item"><a href="{{route('course.show',$item->id)}}">Manage</a></li>
                                                        @if($item->intro=='local')
                                                            <li class="dropdown-item"><a style="cursor: pointer" onclick="OpenMedia('model-{{$item->id}}')">View </a></li>
                                                        @endif
                                                        @if($item->intro=='youtube')
                                                        <li class="dropdown-item"><a style="cursor: pointer" data-toggle="modal" data-target="#model-{{$item->id}}">View </a></li>
                                                            @endif
                                                        ...
                                                        @IFACAN('delete_courses')
                                                        <li class="dropdown-item">
                                                            <a href="#" onclick="RemoveElement('remove-course-{{$item->id}}')">Remove</a>
                                                        </li>
                                                        @ENDACAN
                                                        @IFACAN('create_photos')
                                                        <li class="dropdown-item"><a  href="{{route('upload.form',['course',$item->id])}}">Upload </a></li>
                                                        @ENDACAN
                                                        @IFACAN('read_course_reviews')
                                                        <li class="dropdown-item"><a  href="{{route('course.reviews',$item->id)}}">Reviews </a></li>
                                                        @ENDACAN
                                                        @IFACAN('delete_courses')
                                                              ...
                                                              @if($item->intro=='local')
                                                                <div class="modal-body pt-0">
                                                                    <div class="row no-gutters">
                                                                        ...
                                                                        <div class="col-md-6">
                                                                            <div class="contact-info px-4" style="text-align: justify">
                                                                            ...
                                                                            <span class="mdi mdi-chevron-right-box mdi-18px text-danger"></span> Slug</span> <span>{{$item->slug}}</span>
                                                                                <br>
                                                                                <span class="text-dark font-weight-medium pt-4 mb-2">
                                                                            <span class="mdi mdi-chevron-right-box mdi-18px text-danger"></span> Lessons Hosting</span> <span>{{ucfirst($item->hosting)}}</span>
                                                                                <br>
                                                                                <span class="text-dark font-weight-medium pt-4 mb-2">
                                                                ...
                                                                <div class="modal-body pt-0">
                                                                    <div class="row no-gutters">
                                                                        <div class="col-md-6">
                                                                            <div class="profile-content-left px-4">
                                                                                <div class="card text-center widget-profile px-0 border-0">
                                                                                        <iframe style="height: 200px;border-radius: 15px"  src="https://www.youtube.com/embed/{{$item->video}}?controls=0" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                                                                    <div class="card-body">
